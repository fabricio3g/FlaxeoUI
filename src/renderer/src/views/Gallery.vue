...
<template>
  <div class="flex flex-col h-full">
    ...
    <div class="flex-1 flex overflow-hidden">
      ...
      <div
        v-if="selectedImage"
        class="w-full md:w-80 h-full border-l border-border bg-card flex flex-col shrink-0 absolute inset-0 z-20 md:static md:inset-auto md:z-auto"
      >
        <!-- Preview Image -->
        <div class="flex-1 p-4 flex items-center justify-center overflow-hidden relative">
          <!-- Mobile Close -->
          <button
            @click="selectedImage = null"
            class="absolute top-2 left-2 z-10 p-2 rounded-full bg-black/50 text-white md:hidden hover:bg-black/70"
          >
            <ChevronLeft class="w-5 h-5" />
          </button>
