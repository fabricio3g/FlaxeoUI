  ...
  <Popover v-model:open="open">
    <PopoverTrigger as-child>
      <button
        ...
        title="Recipes — full settings templates"
        aria-label="Recipes"
      >
    ...
    >
      ...
      <div class="space-y-2 border-b border-border/60 px-3 py-2.5">
        <div class="flex gap-1.5">
          <input
            v-model="saveName"
            type="text"
            placeholder="Name current look…"
            class="h-8 min-w-0 flex-1 rounded-md border border-border/70 bg-background px-2 text-xs outline-none focus-visible:ring-2 focus-visible:ring-ring/40"
            @keydown.enter.prevent="saveCurrent"
          />
          <button
            type="button"
            class="shrink-0 rounded-md bg-primary px-2.5 text-xs font-medium text-primary-foreground hover:opacity-90"
            @click="saveCurrent"
          >
            Save
          </button>
        ...
        <div class="flex gap-1">
          <button
            type="button"
            class="inline-flex h-7 flex-1 items-center justify-center gap-1 rounded-md border border-border/70 text-[11px] text-muted-foreground hover:bg-muted hover:text-foreground"
            @click="triggerImport"
          >
            <Upload class="size-3" />
            Import
      ...
      <div class="relative border-b border-border/50 px-3 py-2">
        <Search
        ...
        <input
          ...
          class="h-8 w-full rounded-md border border-border/60 bg-muted/30 pl-8 pr-2 text-xs outline-none focus-visible:ring-2 focus-visible:ring-ring/40"
        />
      </div>
      ...
      <div class="max-h-64 overflow-y-auto p-2">
        ...
        <ul class="space-y-1">
          <li
            ...
            :key="recipe.id"
            class="group rounded-lg border border-transparent px-2 py-1.5 hover:border-border/60 hover:bg-muted/40"
          >
            <button type="button" class="w-full text-left" @click="applyRecipe(recipe.id)">
              <p class="truncate text-xs font-medium text-foreground">{{ recipe.name }}</p>
        ...
        <p
          ...
          class="mb-1 mt-3 px-1 text-[10px] font-medium uppercase tracking-wide text-muted-foreground"
        >
          Yours
        </p>
        <ul class="space-y-1">
          <li
            ...
            :key="recipe.id"
            class="flex items-start gap-1 rounded-lg border border-transparent px-1 py-1 hover:border-border/60 hover:bg-muted/40"
          >
            <button type="button" class="min-w-0 flex-1 px-1 py-0.5 text-left" @click="applyRecipe(recipe.id)">
              <p class="truncate text-xs font-medium text-foreground">{{ recipe.name }}</p>
              <p class="truncate text-[11px] text-muted-foreground">
                {{ preview(recipe.prompt || recipe.description) }}
              </p>
            ...
            <button
              type="button"
              class="aui-icon-button mt-0.5 size-7 shrink-0 rounded-md text-muted-foreground opacity-70 hover:bg-muted hover:opacity-100"
              title="Export"
            ...
            <button
              type="button"
              class="aui-icon-button mt-0.5 size-7 shrink-0 rounded-md text-muted-foreground opacity-70 hover:bg-destructive/10 hover:text-destructive hover:opacity-100"
              title="Delete"
