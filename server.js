...
// Always bind to 0.0.0.0 to allow network access (phones, other devices)
...
if (!fs.existsSync(publicDir)) {
  // Check for sibling directory (when running from out/server/index.js)
...
app.post('/api/start', (req, res) => {
  ...
  if (body.loadMode === 'split') {
    // Split mode is for Flux,Z-Image and More
    if (body.diffusionModel)
      args.push('--diffusion-model', path.join(MODELS_DIR, 'diffusion', body.diffusionModel))
    ...
    if (body.diffusionModel)
      args.push('-m', path.join(MODELS_DIR, 'diffusion', body.diffusionModel))
    if (body.vae) args.push('--vae', path.join(MODELS_DIR, 'vae', body.vae))
  }
  ...
  if (body.controlNet)
    args.push('--control-net', path.join(MODELS_DIR, 'controlnet', body.controlNet))
  if (body.photoMaker)
    args.push('--photo-maker', path.join(MODELS_DIR, 'photomaker', body.photoMaker))
...
app.get('/api/status', (req, res) =>
  res.json({ running: !!sdProcess, logs: serverLogs.slice(-50) })
)
...
app.post('/api/generate-cli', cliUpload, async (req, res) => {
  ...
  if (loadMode === 'split') {
    if (diffusionModel)
      args.push('--diffusion-model', path.join(MODELS_DIR, 'diffusion', diffusionModel))
  ...
  if ((diffusionFa === true || diffusionFa === 'true') && !promptStr.includes('<lora:'))
  ...
  if (diffusionConvDirect === true || diffusionConvDirect === 'true')
    args.push('--diffusion-conv-direct')
  if (vaeConvDirect === true || vaeConvDirect === 'true') args.push('--vae-conv-direct')
  if (forceSDXLVaeConvScale === true || forceSDXLVaeConvScale === 'true')
  ...
  try {
    ...
    // Capture stdout and stderr for serverLogs
    cliProcess.stdout.on('data', (d) => {
      const msg = d.toString()
      process.stdout.write(msg) // Also output to terminal
      serverLogs.push(msg)
    })

    cliProcess.stderr.on('data', (d) => {
      const msg = d.toString()
      process.stderr.write(msg) // Also output to terminal
      serverLogs.push(msg)
    })
    ...
    await new Promise((resolve, reject) => {
      ...
      cliProcess.on('error', (err) => {
        ...
        serverLogs.push(`ERROR: ${err.message}`)
        cliProcess = null
        reject(err)
      })
    })
...
app.post('/api/convert', async (req, res) => {
  ...
  if (!fs.existsSync(sdCliPath)) {
    return res
      .status(500)
      .json({ success: false, error: 'sd-cli not found. Configure backend first.' })
  }
    ...
    try {
      ...
      if (req.files) {
        if (req.files.initImage && req.files.initImage[0]) {
          try {
            fs.unlinkSync(req.files.initImage[0].path)
          } catch (e) {}
        }
...
app.post('/api/generate-video', videoUpload.single('initImage'), async (req, res) => {
  ...
  if (highNoiseDiffusionModel)
    args.push(
      ...
      path.join(MODELS_DIR, 'diffusion', highNoiseDiffusionModel)
    )
      ...
      try {
        ...
        try {
          ...
          if (psOutput.includes('Radeon') || psOutput.includes('AMD')) {
            ...
            psOutput.includes('NVIDIA') ||
            psOutput.includes('GeForce') ||
            psOutput.includes('Quadro')
          ) {
            gpuType = 'cuda12'
            recommendation.note = 'NVIDIA GPU detected. Recommend CUDA 12 build.'
          } else {
...
app.get('/api/backend/config', (req, res) => {
  ...
  const customServerExists =
    fs.existsSync(path.join(CUSTOM_DIR, 'sd-server')) ||
    fs.existsSync(path.join(CUSTOM_DIR, 'sd-server.exe'))
  const customCliExists =
    fs.existsSync(path.join(CUSTOM_DIR, 'sd-cli')) ||
  ...
  try {
    if (fs.existsSync(RELEASES_DIR)) {
      ...
      installedVersions = dirs.filter((dir) => {
        const sdServer =
          fs.existsSync(path.join(RELEASES_DIR, dir, 'sd-server')) ||
          fs.existsSync(path.join(RELEASES_DIR, dir, 'sd-server.exe'))
        const sdCli =
          fs.existsSync(path.join(RELEASES_DIR, dir, 'sd-cli')) ||
          fs.existsSync(path.join(RELEASES_DIR, dir, 'sd-cli.exe'))
        return sdServer && sdCli
      })
    }
  } catch (e) {
    console.error('Error scanning releases:', e)
  }

  // Check active backend
  const activeBackend = getActiveBackendPath()
  const activeServerExists =
    fs.existsSync(path.join(activeBackend, 'sd-server')) ||
    fs.existsSync(path.join(activeBackend, 'sd-server.exe'))
  const activeCliExists =
    fs.existsSync(path.join(activeBackend, 'sd-cli')) ||
...
app.post('/api/backend/download', async (req, res) => {
  ...
  try {
    ...
    console.log(`[Backend] Download complete (size: ${fs.statSync(archivePath).size} bytes), extracting to ${versionDir}...`)

    // Extract using npm packages (cross-platform)
    try {
      if (isTarGz) {
        console.log('[Backend] Extracting tar.gz...')
        ...
        console.log('[Backend] Extracting zip...')
      ...
      console.error('[Backend] Extraction failed:', extractError)
    ...
    try {
      ...
      console.warn('[Backend] Failed to delete archive:', e)
    }

    // Handle nested directories (flatten if necessary)
    // If the zip contained a root folder (e.g. 'bin-v1.0/'), move contents up
    const extractedItems = fs.readdirSync(versionDir)
    const hasBinaries = extractedItems.some(item => item.startsWith('sd-server') || item.startsWith('sd-cli'))
    
    if (!hasBinaries && extractedItems.length === 1 && fs.statSync(path.join(versionDir, extractedItems[0])).isDirectory()) {
      const nestedDir = path.join(versionDir, extractedItems[0])
      console.log(`[Backend] Detected nested directory: ${extractedItems[0]}, flattening...`)
      
      const nestedItems = fs.readdirSync(nestedDir)
      for (const item of nestedItems) {
        const src = path.join(nestedDir, item)
        const dest = path.join(versionDir, item)
        // Handle potential collisions or existing files (though versionDir should be fresh)
        try {
            fs.renameSync(src, dest)
        } catch (mvErr) {
             // Fallback for cross-device rename if needed (though unlikely here)
             console.error(`[Backend] Failed to move ${item}:`, mvErr)
        }
      }
      ...
      try {
        fs.rmdirSync(nestedDir)
         ...
         console.warn('[Backend] Failed to remove nested dir:', rmErr)
      }
    }
...
app.post('/api/backend/set-active', (req, res) => {
  ...
  if (version !== 'custom') {
    ...
    const sdServer =
      fs.existsSync(path.join(versionPath, 'sd-server')) ||
...
const cleanup = async () => {
  ...
  try {
    ...
    console.error('Error stopping cloudflare:', e)
  }
  ...
  if (cliProcess) {
    ...
    try {
      ...
      console.error('Error killing sd-cli:', e)
    }
  }
  process.exit()
}
